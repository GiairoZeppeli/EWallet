...
func (r *SendPostgres) Send(transaction EWallet.Transaction) error {
	...
	getFromAmountQuery := fmt.Sprintf("SELECT amount FROM %s WHERE address='$1' RETURNING amount", walletsTable)
	...
	updateBalanceQuery := fmt.Sprintf("UPDATE %s SET amount = amount+$1 WHERE address=$2", walletsTable)
	_, err = tx.Exec(updateBalanceQuery, transaction.Amount, transaction.To)
	if err != nil {
		tx.Rollback()
